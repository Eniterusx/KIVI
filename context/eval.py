...
from utils import set_seed, get_tinystories, get_wikitext_2, get_wikitext_103, evaluate_distill2, evaluate_wikitext
...
model_name = f"HuggingFaceTB/SmolLM2-360M"
...
if dataset == "wikitext-103":
    ...
    ckp = torch.load(f"{main_dir}/todo")
elif dataset == "wikitext-2":
    _, val_dataset, _ = get_wikitext_2(tokenizer, 512)
    ckp = torch.load(f"{main_dir}/smollm360-wikitext-2/pytorch_model.bin")
    # ckp = torch.load("smollm2_360m_clean.bin")
...
for threshold in mu_pruning_thresholds:
    ...
    if total_mu_params == 0:
        ...
        val_loss, val_bpc, val_ppl = evaluate_distill2(
